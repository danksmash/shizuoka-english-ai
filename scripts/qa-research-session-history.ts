...
assert.ok(html.includes('syncRecentCardHeight'),'desktop recent-session card must synchronize to the left analysis content height');
assert.ok(html.includes('measureAnalysisContentHeight'),'height synchronization must derive the analysis content height independently of the grid row height');
assert.ok(html.includes('last.bottom-first.top'),'height synchronization must measure the visible analysis children rather than the stretched parent box');
assert.equal(html.includes('left.getBoundingClientRect().height'),false,'the stretched parent height must never be used again for synchronization');
assert.ok(html.includes("card.style.maxHeight=height+'px'"),'recent-session card must be explicitly capped to the measured analysis height');
