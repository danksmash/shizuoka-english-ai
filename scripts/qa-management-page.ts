...
const ids = [
  ...
  'researchStudents','researchSessions','researchPeriod','researchMissing','researchRows','researchLine','researchSearch','teacherClass',
];
const elements = Object.fromEntries(ids.map(id => [id, new FakeElement()])) as Record<string, FakeElement>;
elements.u.value = 'qa-user'; elements.p.value = 'qa-password';
...
const fetch = async (path: string) => {
  ...
  if (path === '/api/management/research.csv') {
    researchCsvCalls += 1;
    return response(200, 'research_id,session_id,local_date,total_turns,total_child_words,actual_duration_seconds,reflection_conveyed_ideas,reflection_understood_partner,reflection_noticed_language_culture\nR0001,S1,2026-09-01,4,22,60,5,3,5', 'text/csv');
  }
...
const context = vm.createContext({
  ...
  location: { reload: () => undefined }, alert: () => undefined, console, setTimeout, clearTimeout, Blob,
});
...
console.log('Management role/data isolation QA: PASS');
