...
assert.ok(!feedbackSource.includes('handleCopyReport'), 'Copy button must be removed from report');
...
assert.ok(!appSource.includes('setTimeout(executeTransition,4500)'), 'Farewell must not auto-transition before audio completion');
