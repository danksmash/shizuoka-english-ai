...
insert = '''export function parseReflectionAnswers(value: unknown): ReflectionAnswers | undefined {
  ...
  return {
    ...
    continuedConversation: continuedConversation!,
...
p = Path('src/dataContract.ts')
t = p.read_text()
if marker not in t:
    raise SystemExit('dataContract marker missing')
p.write_text(t.replace(marker, insert + marker, 1))

rep('src/server/persistence.ts',
...
function checkSensitiveLimit(key: string, maxRequests: number, windowMs: number): boolean {
  const now = Date.now();
  const record = sensitiveAttemptMap.get(key);
  if (!record || now > record.resetTime) {
    sensitiveAttemptMap.set(key, { count: 1, resetTime: now + windowMs });
    return true;
  }
...
app.get('/management', (_req, res) => {
  ...
  res.type('html').send(managementPageHtml());
});

app.post('/api/student/resolve', async (req, res) => {
  const ip = req.ip || req.socket.remoteAddress || 'unknown';
  if (!checkSensitiveLimit(`student:${ip}`, 15, 10 * 60_000)) return res.status(429).json({ success: false, error: 'TOO_MANY_CODE_ATTEMPTS' });
  ...
  try {
    ...
    console.error('Student code resolve failed', { message: error?.message });
    return res.status(503).json({ success: false, error: 'STUDENT_LOOKUP_UNAVAILABLE' });
  }
});

app.post('/api/student/history', async (req, res) => {
  const ip = req.ip || req.socket.remoteAddress || 'unknown';
  if (!checkSensitiveLimit(`history:${ip}`, 30, 10 * 60_000)) return res.status(429).json({ success: false, error: 'RATE_LIMITED' });
  const learningCode = normalizeLearningCode(req.body?.learningCode);
  if (!isValidLearningCode(learningCode)) return res.status(400).json({ success: false, error: 'INVALID_LEARNING_CODE' });
  try {
    const student = await resolveStudentByCode(learningCode);
    ...
    console.error('Student history failed', { message: error?.message });
    return res.status(503).json({ success: false, error: 'HISTORY_UNAVAILABLE' });
  }
});

app.post('/api/sessions', async (req, res) => {
  const validated = validateSessionSaveInput(req.body);
  if (!validated.ok) return res.status(400).json({ success: false, error: validated.error });
  try {
    const student = await resolveStudentByCode(validated.value.learningCode);
    if (!student) return res.status(401).json({ success: false, error: 'LEARNING_CODE_NOT_FOUND' });
    const saved = await saveCanonicalSession({
      sessionId: validated.value.sessionId,
      studentId: student.studentId,
      researchId: student.researchId,
      aiStudentId: validated.value.aiStudentId,
      topic: validated.value.topic,
      targetDurationMinutes: validated.value.targetDurationMinutes,
      startedAt: validated.value.startedAt,
      endedAt: validated.value.endedAt,
      history: validated.value.history,
      encounteredVocab: validated.value.encounteredVocab || [],
      reflection: validated.value.reflection,
    });
    res.setHeader('Cache-Control', 'no-store');
    return res.json({ success: true, session: { sessionId: saved.sessionId, lifetimeSessionNumber: saved.lifetimeSessionNumber } });
  } catch (error: any) {
    console.error('Session save failed', { message: error?.message });
    const conflict = error?.message === 'SESSION_ID_CONFLICT';
    return res.status(conflict ? 409 : 503).json({ success: false, error: conflict ? 'SESSION_ID_CONFLICT' : 'SESSION_SAVE_UNAVAILABLE' });
  }
});

app.post('/api/management/login', (req, res) => {
  const ip = req.ip || req.socket.remoteAddress || 'unknown';
  if (!checkSensitiveLimit(`mgmt:${ip}`, 10, 15 * 60_000)) return res.status(429).json({ success: false, error: 'TOO_MANY_LOGIN_ATTEMPTS' });
...
app.get('/api/management/me', requireManagementRole(['teacher','researcher']), (req: AuthenticatedRequest, res) => {
  res.setHeader('Cache-Control', 'no-store');
  return res.json({ success: true, user: req.managementUser });
});
app.get('/api/management/sessions', requireManagementRole(['teacher','researcher']), async (_req, res) => {
  try { const sessions = await getAllSessionsForManagement(); res.setHeader('Cache-Control', 'no-store'); return res.json({ success: true, sessions }); }
  catch (error: any) { console.error('Management sessions failed', { message: error?.message }); return res.status(503).json({ success: false, error: 'MANAGEMENT_DATA_UNAVAILABLE' }); }
});
function csvCell(value: unknown): string { const text = String(value ?? ''); return `"${text.replace(/"/g, '""')}"`; }
app.get('/api/management/research.csv', requireManagementRole(['researcher']), async (_req, res) => {
  try {
    const sessions = await getAllSessionsForManagement();
    const rows = sessions.map(anonymizeSessionForResearch);
    const headers = rows.length ? Object.keys(rows[0]) : ['research_id','session_id'];
    ...
    console.error('Research export failed', { message: error?.message });
...
t = p.read_text()
if marker not in t:
    raise SystemExit('server start marker missing')
