...
requireText(reflection, 'めあてに向かって取り組めた', 'new item 1 wording');
requireText(reflection, '相手の話を聞いて分かろうとしたり，自分の気持ちを伝えようとしたりした', 'new item 2 wording');
forbidText(reflection, '自分の考えをつたえることができた" value={draft.goalRating}', 'superseded entry item 1');
forbidText(reflection, '相手の話を聞いてわかろうとした" value={draft.selfRegulationRating}', 'superseded entry item 2');
requireText(reflection, 'meg-rating-scale-labels', 'rating scale labels');
requireText(reflection, 'できなかった', 'rating scale low anchor');
requireText(reflection, 'よくできた', 'rating scale high anchor');
...
requireText(css, '.meg-rating-scale-labels{display:grid;grid-template-columns:repeat(5,minmax(0,1fr))', 'five-column scale anchors');
requireText(css, '.meg-rating-row{display:grid;grid-template-columns:repeat(5,minmax(0,1fr))', 'five equal rating columns');
...
requireText(reflection, 'selfRegulationRating', 'second rating state');
requireText(reflection, "const RATING_SCHEMA_VERSION = 'v2'", 'current rating schema version');
...
requireText(reflectionApi, 'ratingSchemaVersion: RatingSchemaVersion', 'rating schema sent by current client');
...
requireText(routes, 'ratingSchemaVersion: record.ratingSchemaVersion', 'public schema version');
requireText(routes, 'ratingSchemaVersion: req.body?.ratingSchemaVersion', 'save schema version');
...
for (const field of ['todayGoal','goalRating','selfRegulationRating','ratingSchemaVersion','reflectionText']) requireText(persistence, `${field}:`, 'canonical reflection persistence');
requireText(persistence, "return value === 'v2' ? 'v2' : 'v1'", 'legacy schema normalization');
requireText(persistence, 'existing?.ratingSchemaVersion || ratingSchema(input.ratingSchemaVersion)', 'schema fixed on daily record creation');
...
requireText(teacher, 'めあてへの取組', 'teacher v2 item 1 summary');
requireText(teacher, '聞く・伝える', 'teacher v2 item 2 summary');
...
requireText(teacherApi, 'ratingSchemaVersion?: RatingSchemaVersion', 'teacher API schema version');
...
requireText(teacherModel, "'self_regulation_rating'", 'CSV second rating');
requireText(teacherModel, "'rating_schema_version'", 'CSV rating schema version');
...
console.log('[qa:reflection] PASS: updated reflection points, anchored scale labels, nine hints, versioned rating semantics, storage integrity, privacy, compatibility, and deployment guards verified.');
