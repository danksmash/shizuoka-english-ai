...
requireText(reflection, "aria-label={voiceField === 'todayGoal' ? '今日のめあての音声入力を終了' : '今日のめあてを音声入力'}", 'goal inline voice button');
requireText(reflection, "aria-label={voiceField === 'reflectionText' ? '今日のふりかえりの音声入力を終了' : '今日のふりかえりを音声入力'}", 'reflection inline voice button');
requireText(reflection, "recognition.lang = 'ja-JP'", 'reflection Japanese speech recognition');
requireText(reflection, 'recognition.continuous = true', 'manual-stop continuous reflection speech recognition');
requireText(reflection, 'recognition.interimResults = true', 'live transcript autosave');
requireText(reflection, 'activeRecognition.stop()', 'second-tap voice stop');
requireText(reflection, "setDraft((current) => ({ ...current, [field]: value }))", 'voice transcript enters canonical autosave draft');
forbidText(reflection, 'stableSpeechRecognition', 'reflection must not alter or reuse dialogue ASR');
forbidText(reflection, 'createSpeechRecognitionInstance', 'reflection must remain isolated from dialogue ASR');
...
requireText(css, '.meg-voice-button{width:30px;height:30px', 'inline microphone size without header growth');
requireText(css, '.meg-voice-button.active', 'recording microphone state');
requireText(css, '.meg-voice-button{width:26px;height:26px}', 'short Chromebook microphone size');
