...
# Remove JS listeners for lower shortcut buttons that were intentionally removed from R2.
for fragment in [
...
# Teacher T3: learning ID plus class/attendance must match the rendered data columns.
s=s.replace('<th>学習者ID</th><th>回数</th><th>総対話時間</th>', '<th>学習者ID</th><th>学級・番号</th><th>回数</th><th>総対話時間</th>')

# T6 wording: learner ID is the distributed identifier; reissue changes it while preserving the internal longitudinal link.
...
# T5 breadcrumb must show the learner ID, never the internal UUID.
...
# Replace the current T6 renderer as one function, leaving research logic untouched.
