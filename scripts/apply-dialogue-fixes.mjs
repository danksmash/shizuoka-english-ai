...
// -----------------------------------------------------------------------------
// App.tsx: never request microphone permission on dialogue entry; only microphone
// button starts SpeechRecognition. Persist server-generated child Japanese translation.
// -----------------------------------------------------------------------------
{
  ...
  text = replaceOnce(
    ...
    "  getStudentFarewellMessage,\n} from './utils/speech';",
    'remove requestMicrophonePermission import'
  );
  ...
  text = replaceOnce(
    ...
    "  const handleStartDialogue = (newProfile: StudentProfile) => {\n    setProfile(newProfile);",
    'remove automatic microphone permission request'
  );
...
// -----------------------------------------------------------------------------
// DialogueView.tsx: remove explanatory/cultural note displayed under AI utterances.
// -----------------------------------------------------------------------------
{
...
// -----------------------------------------------------------------------------
// speech.ts: farewell must express gratitude/happiness, and remove pre-authorization
// helper so no code path can proactively invoke getUserMedia.
// -----------------------------------------------------------------------------
{
  ...
  if (farewellStart < 0 || permissionComment < 0 || permissionComment <= farewellStart) {
    throw new Error('Could not locate farewell function block');
  }
...
// -----------------------------------------------------------------------------
// server.ts: normal AI turns must finish with one question, student translation is
// generated by the same model call, and explicit goodbye remains a no-question farewell.
// -----------------------------------------------------------------------------
{
  ...
  text = replaceOnce(
    ...
    `  "reply": "English response from \\${p.name}",\n  "japaneseTranslation": "Natural, gentle Japanese translation of the AI reply suitable for 5th/6th grade student",\n  "studentJapaneseTranslation": "Natural Japanese translation of the student's latest English input; never copy the English input as-is",`,
    'system output schema child translation'
  );
...
`,
  ...
  text = replaceOnce(
    ...
    'enforce question and validate child translation'
  );
