...
const run = (name: string, fn: () => void) => {
  ...
  fn();
  console.log(`Research integrated ${name}: PASS`);
};
...
run('context', () => {
  ...
  assert.ok(data.turns.length > data.sessions.length);
...
run('source', () => {
  ...
  assert.equal(source.includes('SCHOOL_START_MINUTE'), false);
  assert.equal(source.includes('SCHOOL_END_MINUTE'), false);
  ...
  assert.equal(management.includes('教師用管理'), false);
