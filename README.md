...
3. **学校教育向け安全性・プライバシー配慮設計**
   - **Anthropic API** をサーバー経由で安全に連携（APIキーはブラウザに露出しません）
   - **個人情報（PII）の入力防止・事前フィルタリング**: 名前・住所・電話番号等の入力を検知した場合は自動で注意を促し、英語練習へ誘導します（入力内容は安全確認のためチェックされます）
   - **安全な対話範囲の維持**: 小学生の学習に適した英語対話プロンプトおよび出力サニタイズを実装
   - **会話ログの非永続化**: サーバーおよびブラウザストレージに児童の対話履歴を永続保存しません
   - **音声認識・合成の取り扱い**: 本アプリのバックエンドには音声データを送信・保存せず、音声認識結果として得られたテキストを英語対話に利用します
   - **利用にあたって**: 本アプリは学校での英語学習を目的として設計されています。実際の授業等での利用にあたっては、学校・教育委員会のルールに従ってください。
