...
export const SpeechInputBar: React.FC<SpeechInputBarProps> = ({ isRecording, transcript, isAiResponding, onToggleRecording, onSendMessage, onClearTranscript, compact = false }) => {
  const [manualText, setManualText] = useState('');
  const [showKeyboardInput, setShowKeyboardInput] = useState(false);
  const [showPhrases, setShowPhrases] = useState(false);
  useEffect(() => { if (transcript) setManualText(transcript); }, [transcript]);
  const handleSend = () => { const textToSend = manualText.trim(); if (!textToSend || isAiResponding) return; onSendMessage(textToSend); setManualText(''); onClearTranscript(); };
  return <div className={`${compact ? 'p-2.5' : 'p-3 sm:p-4'} bg-white space-y-2.5`}>
    <div className="flex items-center justify-between gap-2">
      <button type="button" onClick={() => setShowPhrases(!showPhrases)} className="min-h-10 sm:min-h-11 px-3 rounded-xl bg-slate-100 text-slate-700 text-xs font-bold flex items-center gap-1.5"><Sparkles className="w-4 h-4 text-blue-600" />お助け{showPhrases ? <ChevronUp className="w-3.5 h-3.5"/> : <ChevronDown className="w-3.5 h-3.5"/>}</button>
      <button type="button" onClick={() => setShowKeyboardInput(!showKeyboardInput)} className="min-h-10 sm:min-h-11 px-3 rounded-xl border border-slate-200 text-slate-600 text-xs font-bold flex items-center gap-1.5"><Keyboard className="w-4 h-4" />文字入力</button>
    </div>
    <AnimatePresence>{showPhrases && <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}} className="overflow-hidden"><div className="flex gap-1.5 overflow-x-auto pb-1">{COMMON_HELP_PHRASES.map((phrase,idx)=><button key={idx} type="button" onClick={()=>{setManualText(phrase.text);setShowKeyboardInput(true);}} className="min-h-10 whitespace-nowrap flex-shrink-0 bg-slate-50 border border-slate-200 text-slate-700 text-xs font-semibold px-3 py-2 rounded-xl">{phrase.text}</button>)}</div></motion.div>}</AnimatePresence>
