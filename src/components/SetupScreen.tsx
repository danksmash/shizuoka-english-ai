...
export const SetupScreen: React.FC<SetupScreenProps> = ({ onStartDialogue, learningDataEnabled, onValidateLearningCode }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-blue-50/40 p-2 text-slate-900 sm:p-3">
      <div className="mx-auto max-w-[1520px] space-y-3">
        ...
        <main className="grid items-start gap-3 xl:grid-cols-[minmax(0,1.03fr)_minmax(0,0.97fr)]">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {AI_STUDENTS_LIST.map((student) => {
                ...
                return <article
                  ...
                  className={`relative flex min-h-[198px] cursor-pointer flex-col rounded-2xl border-2 bg-white p-2.5 shadow-sm transition ${selected ? 'border-blue-600 ring-2 ring-blue-200' : 'border-slate-200 hover:border-blue-300'}`}
                >
                  {selected && <CheckCircle2 className="absolute right-2 top-2 h-5 w-5 fill-blue-600 text-white" />}
                  <div className="mb-2 truncate pr-7 text-[12px] font-black" title={countryLabel(student)}>{student.flag} {countryLabel(student)}</div>
                  <div className="grid flex-1 grid-cols-[84px_minmax(0,1fr)] items-center gap-2.5">
                    <StudentAvatar student={student} size="custom" className="h-[84px] w-[84px] rounded-xl border border-slate-200 object-cover" />
                    <div className="min-w-0">
                  ...
                  <button type="button" onClick={(e) => playPreview(student, e)} className={`mt-2 flex min-h-8 items-center justify-center gap-1 rounded-lg border text-[11px] font-black ${previewPlayingId === student.id ? 'border-blue-600 bg-blue-600 text-white' : 'border-blue-200 bg-white text-blue-700 hover:bg-blue-50'}`}>
          ...
          <section className="min-w-0 space-y-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
              <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_210px] sm:items-center">
                <div className="min-w-0">
                  <p className="text-xl">{selectedStudent.flag}</p>
                  ...
                  <p className="mt-2.5 rounded-xl border border-blue-100 bg-blue-50/70 p-2.5 text-[11px] font-semibold leading-relaxed text-slate-700">{selectedStudent.japaneseBio}</p>
                  <div className="mt-2.5 space-y-1 text-[11px] font-semibold text-slate-700">
                    <p><b>❤️ 好き:</b> {selectedStudent.likes.join('、')}</p>
                    <p><b>🎓 専攻:</b> {selectedStudent.major}</p>
                    <p><b>🏛 名所:</b> {selectedStudent.heritageLandmark}</p>
                  </div>
                </div>
                <StudentAvatar student={selectedStudent} size="custom" className="mx-auto aspect-[4/5] w-full max-w-[210px] rounded-2xl border-2 border-blue-200 object-cover shadow-sm" />
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {DIALOGUE_TOPICS.map((topic, index) => {
                  const selected = selectedTopic === topic.id;
                  return <button key={topic.id} type="button" onClick={() => setSelectedTopic(topic.id)} className={`min-h-11 rounded-xl border px-3 py-2 text-left ${index === DIALOGUE_TOPICS.length - 1 ? 'sm:col-span-1' : ''} ${selected ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-100' : 'border-slate-200 bg-slate-50 hover:bg-slate-100'}`}>
                    <span className="block text-xs font-black">{topic.title}</span>
                    <span className="block text-[10px] font-semibold text-slate-500">{topic.subTitle}</span>
                  </button>;
                })}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
              ...
              <div className="grid grid-cols-4 gap-2">
                {([1, 2, 3, 5] as const).map((minutes) => <button key={minutes} type="button" onClick={() => setDurationMinutes(minutes)} className={`rounded-xl border py-2 text-center ${durationMinutes === minutes ? 'border-blue-600 bg-blue-600 text-white shadow' : 'border-slate-200 bg-slate-50 text-slate-700 hover:bg-blue-50'}`}>
                  <span className="block text-sm font-black">{minutes}分</span><span className="block text-[10px] font-bold opacity-80">{minutes * 60}秒</span>
                </button>)}
              </div>
            </div>

            <button type="button" onClick={handleStart} disabled={checkingCode} className="flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-violet-600 px-6 text-base font-black text-white shadow-md disabled:opacity-60">
              <Play className="h-5 w-5 fill-white" />{checkingCode ? 'コードを確認しています…' : '対話をスタートする！（Start）'}
            </button>
          </section>
        </main>
