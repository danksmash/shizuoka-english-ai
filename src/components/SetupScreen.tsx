...
export const SetupScreen: React.FC<SetupScreenProps> = ({ onStartDialogue, learningDataEnabled, onValidateLearningCode }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-blue-50/40 p-2 text-slate-900">
      <div className="mx-auto max-w-[1560px] space-y-2">
        <header className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 shadow-sm">
          <div className="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="mb-0.5 flex flex-wrap gap-2 text-[10px] font-black">
        ...
        <main className="grid items-start gap-2 xl:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]">
          <section className="min-w-0">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {AI_STUDENTS_LIST.map((student) => {
                ...
                return <article key={student.id} onClick={() => setSelectedStudentId(student.id)} className={`relative flex min-h-[174px] cursor-pointer flex-col rounded-2xl border-2 bg-white p-2.5 shadow-sm transition ${selected ? 'border-blue-600 ring-2 ring-blue-200' : 'border-slate-200 hover:border-blue-300'}`}>
                  ...
                  <div className="grid flex-1 grid-cols-[72px_minmax(0,1fr)] items-center gap-2.5">
                    <StudentAvatar student={student} size="custom" className="h-[72px] w-[72px] rounded-xl border border-slate-200 object-cover" />
                    <div className="min-w-0">
                  ...
                  <button type="button" onClick={(e) => playPreview(student, e)} className={`mt-1.5 flex min-h-7 items-center justify-center gap-1 rounded-lg border text-[11px] font-black ${previewPlayingId === student.id ? 'border-blue-600 bg-blue-600 text-white' : 'border-blue-200 bg-white text-blue-700 hover:bg-blue-50'}`}><Volume2 className="h-3.5 w-3.5" />{previewPlayingId === student.id ? '再生中...' : '声を聞く'}</button>
                </article>;
              })}
            </div>
          </section>
          ...
          <section className="min-w-0 space-y-2">
            <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_170px] sm:items-center">
                <div className="min-w-0">
                  ...
                  <div className="mt-2 grid gap-1 text-[10px] font-semibold text-slate-700 sm:grid-cols-1">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-2.5 shadow-sm">
              ...
              <div className="grid grid-cols-4 gap-2">{([1, 2, 3, 5] as const).map((minutes) => <button key={minutes} type="button" onClick={() => setDurationMinutes(minutes)} className={`rounded-xl border py-1.5 text-center ${durationMinutes === minutes ? 'border-blue-600 bg-blue-600 text-white shadow' : 'border-slate-200 bg-slate-50 text-slate-700 hover:bg-blue-50'}`}><span className="block text-sm font-black">{minutes}分</span><span className="block text-[9px] font-bold opacity-80">{minutes * 60}秒</span></button>)}</div>
            </div>

            <button type="button" onClick={handleStart} disabled={checkingCode} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-violet-600 px-6 text-base font-black text-white shadow-md disabled:opacity-60"><Play className="h-5 w-5 fill-white" />{checkingCode ? 'コードを確認しています…' : '対話をスタートする！（Start）'}</button>
          </section>
        </main>
        <p className="pb-0.5 text-center text-[9px] font-semibold text-slate-500">本アプリは学校での英語学習を目的として設計されています。AI（Anthropic API）を利用した英語対話練習を行います。</p>
      </div>
    </div>
  );
};
