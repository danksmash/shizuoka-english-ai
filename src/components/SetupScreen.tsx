  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-blue-50/40 p-2 text-slate-900">
      <div className="mx-auto max-w-[1560px] space-y-2">
        ...
        <main className="grid items-stretch gap-2 xl:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]">
          <section className="flex min-w-0 flex-col">
            ...
            <div className="grid flex-1 auto-rows-fr grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {AI_STUDENTS_LIST.map((student) => {
                const selected = student.id === selectedStudentId;
                return <article key={student.id} onClick={() => setSelectedStudentId(student.id)} className={`relative flex min-h-[194px] cursor-pointer flex-col rounded-2xl border-2 bg-white p-2.5 shadow-sm transition ${selected ? 'border-blue-600 ring-2 ring-blue-200' : 'border-slate-200 hover:border-blue-300'}`}>
                  ...
                  <div className="grid flex-1 grid-cols-[76px_minmax(0,1fr)] items-center gap-2.5">
                    <StudentAvatar student={student} size="custom" className="h-[76px] w-[76px] rounded-xl border border-slate-200 object-cover" />
                    <div className="min-w-0">
