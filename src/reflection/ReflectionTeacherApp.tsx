...
import {
  ...
  teacherLogout,
  teacherMe,
  teacherShouldRedirectToApiOrigin,
  teacherStudentHistory,
  type TeacherDashboardResponse,
  type TeacherStudentHistoryResponse,
} from './reflectionTeacherApi';

const today = () => new Date().toLocaleDateString('sv-SE');
const fieldLabels: Array<[keyof TeacherStudentHistoryResponse['history'][number], string]> = [
  ['achievements', 'できたこと'], ['languageUsed', '使ったことば'], ['thinking', '授業中に考えていたこと'],
  ['difficultyStrategy', '困ったこと・工夫'], ['languageCultureAwareness', '言葉や文化について気づいたこと'], ['nextGoal', '次に頑張りたいこと'],
];
...
function StudentPanel({ student, onClose }: { student: TeacherStudentHistoryResponse; onClose: () => void }) {
  return <div className="megt-panel-backdrop" onClick={onClose}><aside className="megt-panel" onClick={(e) => e.stopPropagation()}><button className="megt-close" onClick={onClose} aria-label="閉じる">×</button><div className="megt-panel-head"><div><span>{student.classId}</span><h2>{student.learningId}</h2><p>{student.attendanceNumber ? `出席番号 ${student.attendanceNumber}` : '出席番号未設定'}</p></div><BarChart3 /></div>{student.history.length === 0 ? <p className="megt-muted">振り返りはまだありません。</p> : <div className="megt-history">{student.history.map((record) => <article key={record.reflectionId}><div className="megt-history-date"><b>{record.localDate}</b><StatusChip status={record.status === 'submitted' ? 'submitted' : 'draft'} /><span>{record.reflectionCharCount}文字</span></div><h3>今日のめあて</h3><p>{record.todayGoal || '—'}</p>{fieldLabels.map(([key, label]) => { const value = record[key]; return typeof value === 'string' && value ? <div key={String(key)}><h3>{label}</h3><p>{value}</p></div> : null; })}{record.reflectionText && !record.achievements ? <div><h3>旧形式の振り返り</h3><p>{record.reflectionText}</p></div> : null}</article>)}</div>}</aside></div>;
}

export default function ReflectionTeacherApp() {
  const [mode, setMode] = useState<'checking' | 'login' | 'ready'>('checking');
  const [user, setUser] = useState(''); const [localDate, setLocalDate] = useState(today); const [classId, setClassId] = useState('');
  ...
  const checkAuth = async () => {
    try { const me = await teacherMe(); setUser(me.username); setMode('ready'); }
    catch { setMode('login'); }
  };
  const load = async (date = localDate, klass = classId) => {
    setLoading(true); setError('');
    try { setData(await teacherDashboard(date, klass)); }
    catch (e: any) { if (e?.status === 401) setMode('login'); else setError('振り返り一覧を読み込めませんでした。'); }
    finally { setLoading(false); }
  };
  useEffect(() => {
    const redirect = teacherShouldRedirectToApiOrigin();
    if (redirect) { window.location.replace(redirect); return; }
    void checkAuth();
  }, []);
  useEffect(() => { if (mode === 'ready') void load(); }, [mode, localDate, classId]);
  ...
  const exportCsv = async () => { try { await teacherExportCsv(localDate, classId); } catch { setError('CSVを書き出せませんでした。'); } };
  ...
  return <div className="megt-app"><header className="megt-header"><div className="megt-brand"><div className="megt-logo"><BarChart3 /></div><div><h1>My English Growth</h1><p>教師用 振り返りダッシュボード</p></div></div><div className="megt-user"><span>{user}</span><button onClick={logout}><LogOut />ログアウト</button></div></header>
    <main className="megt-main"><section className="megt-toolbar"><label><CalendarDays />日付<input type="date" value={localDate} onChange={(e) => setLocalDate(e.target.value)} /></label><label><Users />学級<select value={classId} onChange={(e) => setClassId(e.target.value)}><option value="">すべて</option>{(data?.classes || []).map((klass) => <option key={klass} value={klass}>{klass}</option>)}</select></label><label className="megt-search"><Search />ID検索<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="例：6RSX" maxLength={8} /></label><button className="megt-secondary" onClick={() => void load()} disabled={loading}><RefreshCw />更新</button><button className="megt-export" onClick={exportCsv}><Download />CSV</button></section>
      ...
      <section className="megt-table-card"><div className="megt-table-head"><div><FileText /><h2>{data?.localDate || localDate} の振り返り</h2></div><p>文字数は学習評価ではなく、記述状況を把握するための補助表示です。</p></div><div className="megt-table-scroll"><table><thead><tr><th>学級</th><th>出席</th><th>ID</th><th>状況</th><th>文字数</th><th>今日のめあて</th><th>できたこと</th><th>次に頑張りたいこと</th></tr></thead><tbody>{filtered.map((row) => <tr key={`${row.classId}-${row.learningId}`} onClick={() => void openStudent(row.learningId)} tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') void openStudent(row.learningId); }}><td>{row.classId}</td><td>{row.attendanceNumber || '—'}</td><td><b>{row.learningId}</b></td><td><StatusChip status={row.status} /></td><td>{row.reflectionCharCount || '—'}</td><td>{row.todayGoal || '—'}</td><td>{row.achievements || '—'}</td><td>{row.nextGoal || '—'}</td></tr>)}</tbody></table>{!loading && filtered.length === 0 && <div className="megt-empty">該当する児童はいません。</div>}</div></section>
