...
function speakStudentVoiceLocal(
  ...
  const config = PERSONA_VOICE_MAP[student.id] || {
    preferredNames: [], fallbackLang: student.voiceLang || 'en-US',
    defaultPitch: student.gender === 'female' ? 1.25 : 1.0, defaultRate: 0.9,
  };
  ...
  utterance.onerror = (err) => { console.warn('Local TTS error:', err); onError?.(err); onEnd?.(); };
  try { window.speechSynthesis.speak(utterance); }
  catch (e) { console.warn('Error initiating local TTS:', e); onError?.(e); onEnd?.(); }
  return utterance;
}

/**
 * AI-student speech is Google Cloud Chirp 3 HD first. If Cloud TTS is unavailable,
 * the existing device SpeechSynthesis path remains as a classroom-safe fallback.
 */
...
export function rebuildSpeechRecognitionSnapshot(
  ...
  const pieces = results.map((item) => ({ transcript: String(item.transcript || '').trim(), isFinal: Boolean(item.isFinal) })).filter((item) => item.transcript.length > 0);
  let combinedRaw = '';
...
export function createSpeechRecognitionInstance(
  ...
  try {
    ...
    recognition.stop = () => {
      stopRequested = true;
      try { return nativeStop(); }
      catch (error) { if (isAndroidDevice && hasRecognizedText) return; throw error; }
    };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    recognition.onresult = (event: any) => {
      ...
      } catch (e) { console.warn('Speech onresult error:', e); }
    };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    recognition.onerror = (event: any) => { console.warn('Speech recognition error:', event.error); onError(event.error || '音声認識エラーが発生しました'); };
    recognition.onend = () => {
      if (isAndroidDevice && !stopRequested && hasRecognizedText) return;
      onEnd();
    };
    return recognition;
  } catch (err) {
