...
(function(){
  ...
  function stackedClassBars(rows,legend){
    var items=Array.isArray(rows)?rows.slice(0,14):[];
  ...
  renderDashboard=function(d,appliedQuery){
    ...
    if(chart)chart.innerHTML=stackedClassBars(rows,charts.dailyClassLegend||[]);
  };
