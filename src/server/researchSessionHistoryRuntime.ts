...
@media(max-width:760px){.rsh-panel{width:100vw}.rsh-detail-meta{grid-template-columns:1fr}.rsh-utterance{grid-template-columns:42px 40px minmax(0,1fr)}.rsh-modal-backdrop{padding:0}.rsh-modal{width:100vw;max-height:100vh;height:100vh;border-radius:0}.rsh-modal-tools{flex-direction:column;align-items:stretch}}
</style>`;
...
(function(){
  ...
  function ensureUi(){
    var card=document.querySelector('.recent-card');if(card&&!h$('rshRecentSearch')){var wrap=card.querySelector('.table-wrap');var bar=document.createElement('div');bar.className='rsh-toolbar';bar.innerHTML='<input id="rshRecentSearch" type="search" placeholder="表示中50件を research_id / session_id で検索"><button id="rshAllBtn" class="secondary" type="button">全セッション一覧</button>';card.insertBefore(bar,wrap);var count=document.createElement('div');count.id='rshRecentCount';count.className='rsh-count';count.textContent='最新50件まで表示';card.insertBefore(count,wrap);h$('rshRecentSearch').addEventListener('input',renderRecent);h$('rshAllBtn').addEventListener('click',openAll)}
  ...
  async function openHistory(researchId,sessionId){ensureUi();openPanel();h$('rshPanelTitle').textContent='research_id '+researchId;h$('rshPanelSub').textContent='匿名化された同一児童のAI対話履歴';h$('rshPanelStatus').textContent='履歴を読み込んでいます…';h$('rshHistoryList').innerHTML='';h$('rshSessionDetail').innerHTML='セッション詳細を読み込んでいます…';try{var data=historyCache.get(researchId);if(!data){data=await api('research.session-history',{researchId:researchId});historyCache.set(researchId,data)}renderHistory(data,sessionId);h$('rshPanelStatus').textContent='保存されている対話 '+String(data.count||0)+'回';if(sessionId)loadDetail(researchId,sessionId)}catch(e){h$('rshPanelStatus').textContent='履歴を読み込めません: '+e.message;h$('rshPanelStatus').className='status error';h$('rshSessionDetail').innerHTML=''}}
  ...
  ensureUi();if(!hookRender()){var tries=0,timer=setInterval(function(){tries+=1;if(hookRender()||tries>40)clearInterval(timer)},100)}
